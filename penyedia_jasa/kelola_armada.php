        ...
        <div class="row mb-3">
            <div class="col-md-12">
              <?php
                ...
                ?>
                  <a href="tambah_armada.php" class="btn btn-primary btn-sm float-right"><i class="fa fa-plus-circle mr-2"></i>Tambah</a>
                <?php
                }else {
                ?>
                  <div></div>
            ...
            <div class="card">              
              <div class="card-body">
                <table id="example1" class="table table-bordered table-striped">
                  ...
                  <tbody>
                    <?php                    
                      $q_tabel = mysqli_query($connect, "SELECT * FROM tb_armada WHERE id_pj='".$profil['id_pj']."' ORDER BY id_armada DESC");                      
                      $no = 0;
                    ...
                    <tr>
                      <td><?php echo $no; ?></td>
                      <td><?php echo $tabel['namaArmada']; ?></td>
                      <td><?php echo $tabel['kotaTujuan']; ?></td>
                      <td><?php echo $tabel['poolTujuan']; ?></td>
                      <td><?php echo $tabel['jamBerangkat']; ?></td>
                      <td><?php echo $tabel['jamTiba']; ?></td>
                      <td><?php echo $tabel['kapasitasKursi'].' kursi'; ?></td>
                      <td><?php 
                            if($tabel['harga']<1000){
                              $tabel['harga']=$tabel['harga']*1000;
                              echo 'Rp. '. number_format($tabel['harga']);       
                            }else{                                
                              echo 'Rp. '. number_format($tabel['harga']);
                            } 
                          ?>
                      ...
                      <td>
                        <a href="ubah_armada.php?id_armada=<?php echo $tabel['id_armada']; ?>" class="btn btn-block btn-primary btn-xs" title="Ubah"><i class="far fa-edit"></i></a>
                        <button type="button" class="btn btn-block btn-danger btn-xs" title="Hapus" data-toggle="modal" data-target="#modal-sm<?php echo $tabel['id_armada']; ?>">
                            <i class="far fa-trash-alt"></i>
                            </button>

                            <div class="modal fade" id="modal-sm<?php echo $tabel['id_armada']; ?>">
                              <div class="modal-dialog modal-sm">
                                <div class="modal-content">
                                  ...
                                  <div class="modal-body text-center">                      
                                      <p>Yakin ingin menghapus armada ini?</p>                    
                                  </div>
                                  <div class="modal-footer justify-content-between">
                                    <button type="button" class="btn btn-default" data-dismiss="modal">Batal</button>
                                    <button id_armada="<?php echo $tabel['id_armada']; ?>" type="button" data-dismiss="modal" class="btn btn-primary ajaxhapus">Ya</button>                                   
                                  </div>            
                                </div>
                                <!-- /.modal-content -->
                              </div>
...
<script>
  $('.ajaxhapus').click(function(){
    var id_armada = $(this).attr("id_armada");
    hapus_pesanan(id_armada);
  });
  function hapus_pesanan(id_armada) {
    $.ajax({
      type: "get",
      url: "proses_hapus_armada.php?",
      data: "id_armada="+id_armada,          
    }).done(function() {
      swal({ 
            title: "Berhasil Dihapus!",
            text: "Data Armada berhasil dihapus!",
            type: "success",
