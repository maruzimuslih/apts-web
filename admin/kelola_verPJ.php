        ...
        <ul class="nav nav-pills nav-sidebar flex-column nav-child-indent" data-widget="treeview" role="menu" data-accordion="false">
          ...
          <li class="nav-item has-treeview menu-open">
            ...
            <ul class="nav nav-treeview">
              ...
              <li class="nav-item">
                <a href="kelola_verPJ.php" class="nav-link active">
                  <i class="far fa-circle nav-icon"></i>
  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1 class="m-0 text-dark">Verifikasi Pengguna (Penyedia Jasa)</h1>
          </div><!-- /.col -->
          <div class="col-sm-6">
            ...
            <div class="card">              
              <div class="card-body">
                <table id="example1" class="table table-bordered table-striped">
                  ...
                  <tbody>
                    <?php                                       
                      $q_tabelPJ = mysqli_query($connect, "SELECT * FROM tb_pj JOIN tb_user ON tb_pj.id_user=tb_user.id_user WHERE statusPJ=0 ");                      
                      $no = 0;
                      while($tabel = mysqli_fetch_array($q_tabelPJ)){
                        $no++;                    
                    ?>
                    <tr>
                    <td><?php echo $no; ?></td>
                      <td><?php echo $tabel['namaPJ']; ?></td>                                                            
                      <td><?php echo $tabel['email']; ?></td>                                            
                      <td>
                        <button type="button" class="btn btn-block btn-info btn-xs" title="Lihat" data-toggle="modal" data-target="#modal-default<?php echo $no; ?>">
                          Lihat
                        </button>

                        <div class="modal fade" id="modal-default<?php echo $no; ?>">
                            <div class="modal-dialog">
                            <div class="modal-content">
                                <div class="modal-header">
                                <h4 class="modal-title">Bukti Pendukung</h4>
                                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                    <span aria-hidden="true">&times;</span>
                                </button>
                                </div>
                                <div class="modal-body text-center">                      
                                    <embed id="bukti" src="../bukti_instansi/<?php echo $tabel['buktiPendukung'];?>" style="max-width:100%;" alt="Bukti Pendukung">                     
                                </div>            
                            </div>
                            <!-- /.modal-content -->
                            </div>
                      ...
                      <td>                          
                        <a href="proses_verPJ.php?id_pj=<?php echo $tabel['id_pj']; ?>&status=1" class="btn btn-block btn-success btn-xs" title="Verifikasi">Verifikasi</a>
                        <a href="proses_verPJ.php?id_pj=<?php echo $tabel['id_pj']; ?>&status=2" class="btn btn-block btn-danger btn-xs" title="Tolak">Tolak</button>                                                                                                    
                      </td>                                            
                    </tr>
                    <?php
                      }
...
<script>
  $('.ajaxhapus').click(function(){
    ...
    hapus_pengguna(email);
  });
  function hapus_pengguna(email) {
    $.ajax({
      type: "get",
